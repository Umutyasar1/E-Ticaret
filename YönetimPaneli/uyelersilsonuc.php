...
if(isset($_SESSION["Yonetici"])){
	...
	if($GelenID!=""){
		$UyeSilmeSorgusu	=	$Connection->prepare("UPDATE uyeler SET SilinmeDurumu = ? WHERE id = ? LIMIT 1");
		$UyeSilmeSorgusu->execute([1, $GelenID]);
		$UyeSilmeKontrol	=	$UyeSilmeSorgusu->rowCount();

		if($UyeSilmeKontrol>0){
			$SepetSilmeSorgusu		=	$Connection->prepare("DELETE FROM sepet WHERE UyeId = ?");
			$SepetSilmeSorgusu->execute([$GelenID]);
	
			$YorumlarSorgusu	=	$Connection->prepare("SELECT * FROM yorumlar WHERE UyeId = ?");
			...
			if($YorumlarSayisi>0){
				foreach($YorumlarKayitlari as $YorumSatirlari){
					...
					$YorumSilmeSorgusu		=	$Connection->prepare("DELETE FROM yorumlar WHERE id = ? LIMIT 1");
