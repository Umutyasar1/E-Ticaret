...
if(isset($_SESSION["Yonetici"])){
	...
	if(($GelenID!="") and ($GelenBannerAlani!="") and ($GelenBannerAdi!="")){
		$BannerResmiSorgusu		=	$Connection->prepare("SELECT * FROM bannerlar WHERE id = ? LIMIT 1");
		...
		if($GelenBannerAlani==$BannerBilgisi["BannerAlani"]){		
			$BannerGuncellemeSorgusu		=	$Connection->prepare("UPDATE bannerlar SET BannerAlani = ?, BannerAdi = ? WHERE id = ? LIMIT 1");
					...
					if($BannerYukle->uploaded){
					   ...
					   $BannerYukle->process($VerotIcinKlasorYolu);
						...
						if($BannerYukle->processed){
							$BannerResmiGuncellemeSorgusu	=	$Connection->prepare("UPDATE bannerlar SET BannerResmi = ? WHERE id = ? LIMIT 1");
				...
				$BannerYukle	=	new upload($GelenBannerResmi, "tr-TR");
					if($BannerYukle->uploaded){
					   ...
					   $BannerYukle->process($VerotIcinKlasorYolu);
						...
						if($BannerYukle->processed){
							$BannerResmiGuncellemeSorgusu	=	$Connection->prepare("UPDATE bannerlar SET BannerAlani = ?, BannerAdi = ?, BannerResmi = ? WHERE id = ? LIMIT 1");
