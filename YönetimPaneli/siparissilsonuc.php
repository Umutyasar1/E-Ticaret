...
if(isset($_SESSION["Yonetici"])){
	...
	if($GelenSiparisNo!=""){
		$SiparislerSorgusu		=	$Connection->prepare("SELECT * FROM siparisler WHERE SiparisNumarasi = ?");
		...
		if($SiparisKontrol>0){
			foreach($SiparisKayitlari as $Satirlar){
				...
				$SiparisSilmeSorgusu	=	$Connection->prepare("DELETE FROM siparisler WHERE id = ? LIMIT 1");
				$SiparisSilmeSorgusu->execute([$SiparistekiId]);
				$SilmeKontrol			=	$SiparisSilmeSorgusu->rowCount();

				if($SilmeKontrol>0){
					$UrunGuncellemeSorgusu	=	$Connection->prepare("UPDATE urunler SET ToplamSatisSayisi=ToplamSatisSayisi+? WHERE id = ? LIMIT 1");
					...
					if($UrunGuncellemeKontrol>0){
						$VaryantGuncellemeSorgusu	=	$Connection->prepare("UPDATE urunvaryantlari SET StokAdedi=StokAdedi+? WHERE VaryantAdi = ? AND UrunId = ? LIMIT 1");
